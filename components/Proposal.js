...
const Proposal = ({ id, votingContract, address, web3 }) => {
  ...
  return (
    <div>
      {proposalInfo && (
        <div>
           <div key={id} className="border shadow rounded-xl overflow-hidden">
                <div className="p-4">
                  <p style={{ height: '64px' }} className="text-2xl font-semibold">{proposalInfo.description}</p>
                  <div style={{ height: '70px', overflow: 'hidden' }}>
                    <p className="text-gray-400">{proposalInfo.timestamp}</p>
                  </div>
                </div>
